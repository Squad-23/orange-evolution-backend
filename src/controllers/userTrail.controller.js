...
const getUserSubscribedTrails = async (req, res) => {
    try {
        ...
        const trails = await Promise.all(user.trails.map(async (trail) => {
            if(trail) {
                let modules = await moduleService.findByTrailIdModuleService(trail._id);
                ...
                modules = modules.map(module => {
                        ...
                        if (subject) {
                            const contents = module.contents
                                ...
                                .map(content => ({
                                    ...
                                    link: content.link,
                                    time: content.time,
                                    completed: user.completeds.includes(content._id),
                                }));

                            const completeds = contents.filter(content => content.completed);
                    ...
                    return ({
                        ...
                        progress: {
                            max: subjects.length,
                            completed: subjects.filter(({ progress }) => progress.max && progress.completed === progress.max).length
                        },
                        subjects,
                    })
                })
            ...
            return ({
                ...
                progress: {
                    max: modules.length,
                    completed: modules.filter(({ progress }) => progress.max && progress.completed === progress.max).length
                },
                modules
            })
            }
